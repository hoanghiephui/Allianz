...
import com.allianz.android.BuildConfig
import com.allianz.android.api.EnvironmentSettings
import com.allianz.android.utils.TLSSocketFactory
...
class ApiModule {
    ...
    @Provides
    @Named("explorer")
    fun provideRetrofitExplorerInstance(okHttpClient: OkHttpClient,
                                        converterFactory: GsonConverterFactory,
                                        rxJavaCallFactory: RxJava2CallAdapterFactory,
                                        environmentSettings: EnvironmentSettings): Retrofit {
        return Retrofit.Builder()
                .baseUrl(environmentSettings.explorerUrl)
                .client(okHttpClient)
                .addConverterFactory(converterFactory)
                .addCallAdapterFactory(rxJavaCallFactory)
                .build()
